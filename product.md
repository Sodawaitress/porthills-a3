...
- US6 结果（改进版）：`bare_rock` 只有7个训练点、没有完整边界（⚠️之前说它"有现成多边形可以叠掩膜"是错的，已核实撤回——查了整个gdb确认从未有过bare_rock多边形），剔除出分类训练集，分类器只学5类 → **OA=0.742, Kappa=0.676**（比6类版本 OA=0.710/Kappa=0.638 全面提高）。`broadleaf_scrub`→`gorse_broom` 错7/20 的老问题还在，证实是这两类本身光谱重叠，不是bare_rock拖累的假象。全栅格地图上bare_rock仍是已知空白，US7/US9要另外处理。
