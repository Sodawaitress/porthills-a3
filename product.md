...
## 🔵 现在做：US4 收尾 — 散点图 + 精度评论文字

- ✅ US1+US2+US3 全部完成。
- ✅ **决定#2 定了**：dNBR 连续回归。相关表、VIF清理(迭代删到6个变量全部达标)、最终回归(R²=0.41→加FuelClass到0.48)全部跑完，数字都在 report.md R4 段落里。
- ✅ 意外发现：FuelClass 类别变量显著提升模型(p=0.0004)，跟2024 Utah那篇文献结论相反，是个能写的讨论点。

**下一步**：补散点矩阵图（R4"散点"5分还没做），然后 US4 就能收尾，转 US5(可分性)。
