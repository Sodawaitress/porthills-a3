...
## 🔵 现在做：US4 — §4 关系（相关/散点/回归/VIF）

- ✅ US1（训练点，6类211个）+ US2（点表）+ US3（数据探索：离群值/正态性/变换）全部完成，见下方对应行。
- ✅ 核心点表 `scripts/PortHills_PointTable.csv`，211 点 × 22 波段，干净可用。

**下一步**：US4 —— 相关表、散点矩阵、回归（决定#2：dNBR线性回归 vs 类别逻辑回归，还没定）、VIF 多重共线性检查。
