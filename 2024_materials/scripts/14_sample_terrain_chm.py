"""
2024 Port Hills fire - sample terrain (elevation/slope/northness) + CHM at the
250 training points. Fully local (DEM/DSM already on J:, no GEE needed).

Uses ExtractMultiValuesToPoints instead of arcpy.sa.Sample() - 2017's US2 hit
a real bug where Sample()'s auto-generated OBJECTID field got confused with
the real point ID after some points had been deleted, silently misaligning
labels and band values for half the rows. ExtractMultiValuesToPoints writes
straight onto the input features in place, so there's no separate ID field to
...
aspect = arcpy.sa.Aspect(dem_clip)                       # degrees, -1 = flat
# northness = cos(aspect in radians); flat (-1) -> 0 (no directional signal)
northness = arcpy.sa.Con(aspect == -1, 0,
                          arcpy.sa.Cos(aspect * 3.14159265358979 / 180.0))
northness_path = f"{out_dir}\\figures\\Northness_2024_AOI.tif"
northness.save(northness_path)
elev_path = f"{out_dir}\\figures\\Elevation_2024_AOI.tif"
dem_clip.save(elev_path)

slope_path = f"{out_dir}\\figures\\Slope_2024_AOI.tif"           # already made by 09
chm_path = f"{out_dir}\\figures\\CHM_2024_AOI_capped.tif"        # already made by 04

pts_fc = f"{out_dir}\\TrainingPoints_2024_raw.shp"
arcpy.sa.ExtractMultiValuesToPoints(pts_fc, [
    [elev_path, "elev"],
    [slope_path, "slope"],
    [northness_path, "northness"],
    [chm_path, "chm"],
])

n_null = 0
with arcpy.da.SearchCursor(pts_fc, ["elev", "slope", "northness", "chm"]) as cur:
    for row in cur:
        if any(v is None for v in row):
            n_null += 1
print(f"Points with any null terrain/CHM value: {n_null} / {int(arcpy.management.GetCount(pts_fc)[0])}")

# ---- export a local terrain+CHM table (join key = FID, matches point order) ----
csv_path = f"{out_dir}\\TrainingPoints_2024_terrain.csv"
with open(csv_path, "w") as f:
    f.write("FID,FuelClass,class_id,split,block_id,elev,slope,northness,chm\n")
    with arcpy.da.SearchCursor(pts_fc, ["FID", "FuelClass", "class_id", "split", "block_id",
                                         "elev", "slope", "northness", "chm"]) as cur:
        for row in cur:
