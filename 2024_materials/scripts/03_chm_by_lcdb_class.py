"""
2024 Port Hills fire - CHM (canopy height model) by LCDB Name_2023 class, inside the AOI.
Purpose: LCDB's "Forest - Harvested" label (post-2017-fire disturbance, LCDB has no
dedicated burned-forest class) is ambiguous about what's actually there now -
regenerating pine, gorse invasion, or still-bare ground. Canopy height from LiDAR
should separate these even before any aerial-photo check.

Data:
  DSM/DEM: J:\\Data\\Digital_Elevation_Models\\Christchurch_LiDAR_2021-2022\\CHC_LiDAR_2020_21_{DSM,DEM}.tif
           LINZ "Canterbury - Christchurch 1m DEM/DSM (2020-2021)", flown 2020-12,
           full clean coverage over this AOI (verified: valid cell count matches
           polygon area to within 0.01%).
  LCDB:    J:\\Data\\Landcover_Database_v6\\...shp, Name_2023 field.
  AOI:     2024 Port Hills fire official boundary (ECan, from 2024_materials/boundary).

Author: Claude (for Yu Zhou), 2026-09-20
"""
import arcpy
from arcpy.sa import Raster, Minus
import numpy as np
...
poly_2024_wgs84 = arcpy.Polygon(array_2024, wgs84)
poly_2024_nztm = poly_2024_wgs84.projectAs(nztm)
arcpy.management.CopyFeatures(poly_2024_nztm, "in_memory/aoi_2024")

dsm_path = r"J:\Data\Digital_Elevation_Models\Christchurch_LiDAR_2021-2022\CHC_LiDAR_2020_21_DSM.tif"
dem_path = r"J:\Data\Digital_Elevation_Models\Christchurch_LiDAR_2021-2022\CHC_LiDAR_2020_21_DEM.tif"
lcdb_path = r"J:\Data\Landcover_Database_v6\lcdb-v60-land-cover-database-version-60-mainland-new-zealand.shp"

dsm = arcpy.sa.ExtractByMask(dsm_path, "in_memory/aoi_2024")
dem = arcpy.sa.ExtractByMask(dem_path, "in_memory/aoi_2024")
chm = Minus(dsm, dem)
chm_path = r"C:\Users\zhouy3d\Desktop\a3\2024_materials\figures\CHM_2024_AOI.tif"
chm.save(chm_path)
print("CHM saved:", chm_path)

arcpy.analysis.Clip(lcdb_path, "in_memory/aoi_2024", "in_memory/lcdb_clip")

zonal_table = "in_memory/chm_by_class"
arcpy.sa.ZonalStatisticsAsTable("in_memory/lcdb_clip", "Name_2023", chm, zonal_table,
                                 statistics_type="ALL")

print("\n=== CHM (m) by LCDB Name_2023 class, inside 2024 fire AOI ===")
...
# also: what fraction of pixels in "Forest - Harvested" are near-zero height (open/shrub-like, <2m)
# vs meaningfully tree-height (>5m, plausible regenerating/residual pine)
arr_chm = arcpy.RasterToNumPyArray(chm, nodata_to_value=-9999)
