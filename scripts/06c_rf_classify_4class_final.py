"""
US6 最终版 - 4类RF分类，排除 bare_rock(7点,无法学出稳定边界) 和
cleared_pine(修正后19点,3-4个小图斑,空间分块切分下几乎学不到东西,PA=0.067)。
两者都改用已知边界(bare_rock=承认是空白,cleared_pine=cleared_pine_real多边形)
在最终地图/refugia分析里直接叠加，不进分类训练集。
"""
import pandas as pd
...
print("4-class total:", len(df4))
print(df4["FuelClass"].value_counts())
...
for i, c in enumerate(classes):
    ...
    print(f"  {c:16s}  PA(漏分)={pa:.3f}  UA(错分)={ua:.3f}  (n_valid={row_sum})")
...
axes[0].set_title(f"4-class Confusion Matrix (bare_rock + cleared_pine masked)\nOA={oa:.2f} Kappa={kappa:.2f}")
...
axes[1].set_title("Feature importance (4-class final model)")
axes[1].invert_yaxis()

plt.tight_layout()
out_png = os.path.join(OUT_DIR, "rf_confusion_matrix_4class_final.png")
