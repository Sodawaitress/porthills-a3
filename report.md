...
- ⭐ **way forward 内容草稿（如果以后拿一场更近期的火重跑这套流程，哪些会真的变好、哪些不会）**——不是空想，是基于这次项目里实际撞过的具体限制推出来的：

  **全流程一句话总结**：火前Landsat 8合成(云掩膜+缩放) → LCDB `Class_2012`+航片判读做训练标签(6类，`cleared_pine`用Hansen森林损失∩exotic_pine定义) → 15m像元纯度检查 + 300m空间分块70/30切分(防空间自相关) → §3探索(IQR离群值+按类正态性+变换) → §4关系(相关/VIF清理/线性回归预测dNBR，R²=0.41-0.48) → §5可分性(JM指数) + RF分类(4类，OA=0.684) → 全栅格套用分类器+裁真实火场边界 → refugia(severity=0)跟植被类型/地形交叉分析。

  **会真的变好的地方**（本项目里已经具体验证过的数据缺口，2025年的火大概率不会有）：
  - **Sentinel-2 地表反射率(L2A)**：本项目AOI在2017年3月前没有S2大气校正产品，火前只能用Landsat 8，没有红边波段；已经测过红边波段对`gorse_broom`/`broadleaf_scrub`这对分不开的类确实有帮助(JM从1.459升到1.71)。2025年的火火前火后都能拿到完整S2 L2A时间序列，不只是补一个红边波段，还能做真正的物候时间序列，不是像本项目这样东拼西凑几张影像。
  - **火后云污染**：本项目`cleared_pine`19个点里16个撞上火后影像云层，只能放弃这批dNBR数据。2025年卫星更多(S2A/B/C+Landsat8/9)，火后几天内找到一张干净影像的概率高得多，这类"数据存在但被云挡住"的损失能实质性减少。
  - **同期LiDAR**：本项目测冠层高度(CHM)时，能用的LiDAR不是火后3-4年(有反向因果风险)就是火前2年但覆盖有缺口——两个都不理想。2025年的火如果LiDAR恰好在火前不久飞过，就能拿到真正干净的火前冠层结构数据，不用像本项目这样两难取舍。
  - **训练点规模**：本项目`cleared_pine`修正后只有19个点、`bare_rock`只有7个——不是方法问题，是这次这片AOI里这两类地表本来就稀少。换一场火，只要新的AOI里这些地表面积更大，样本量瓶颈自然缓解，不需要额外的方法学突破。

  **不会变好的地方**（这次遇到的是结构性限制，不是"数据老了"）：
  - **`gorse_broom`/`broadleaf_scrub`光谱混淆**：查过文献(shrubland分类研究)，这类灌木在光谱上本来就像，专业研究就算用最好的预测变量集也做不到完全分开，只有像元100%纯的情况下才行；真正有用的是冠层结构/纹理(比如CHM+纹理能带来~14%准确率提升)，不是"换一批更新的影像"就能解决——2025年一样会遇到这个问题，除非额外加LiDAR结构或高分辨率纹理特征。
  - **天气/燃料含水率解释不了像元级差异**：本项目已经论证过，这类变量在几平方公里的AOI内基本均匀，能解释"这场火整体多凶"，解释不了"同一场火里像元之间的差异"——这是空间尺度不匹配，不是数据新旧的问题，2025年的火同样解决不了。
