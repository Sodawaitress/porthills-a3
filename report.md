...
- ⭐ **way forward 内容草稿——可迁移的方法 + 需要什么数据才能做得更好**（不对未知的未来事件断言"能/不能解决"，只讲这次验证过什么、缺什么数据、有了会怎样）：
  ...
  **需要什么数据、有了会带来什么改善**（基于这次项目里具体缺过的东西，不是泛泛而谈）：
  ...
  - **火前几个月内飞行的LiDAR(冠层高度+纹理)**：本项目测过的两版CHM都有时间错位问题(火后3-4年有反向因果风险；火前2年覆盖不全)。**如果有真正贴近火前的LiDAR**，文献显示CHM+纹理能带来约14%的分类准确率提升——这正是`gorse_broom`/`broadleaf_scrub`光谱混淆问题最值得优先尝试的方向，不是没救，是需要这类数据才能真正测出来。
  ...
  - **细粒度(而非站点尺度)燃料含水率/风场数据**：本项目论证过站点级天气数据在几平方公里AOI内太均匀，解释不了像元级差异。**如果能拿到高分辨率(如WRF-Fire之类)模拟的风场/含水率场**，理论上能给回归模型补上一个目前完全缺失的变量类别，这条没有验证过是否真的有直接可用的数据源，值得作为下一步调研方向，而不是断言做不到。
