...
- ⭐⭐ **主模型决定：S+Sp（火前光谱 10 变量 + 2016 春季 9 变量），不用地形当分类器输入**（2026-09-26 定，🤖`R5_shap_run/07_compare_varsets.py` + zip 里 `Tab5_7_cv_summary.csv`）。四套变量集在**同一套 171 点 / 300m 空间分块 / 5 折交叉验证**上并列比(10 个随机种子)：

  | 变量集 | 变量数 | CV OA(均值±sd) | 10 种子夺冠 |
  |---|---|---|---|
  | S 只光谱 | 10 | 0.594–0.598 | 0 |
  | S+T +地形 | 13 | 0.617–0.618 | 0 |
  | **S+Sp +春季** | 19 | **0.642–0.643** | 5 |
  | S+T+Sp 全部 | 22 | 0.645–0.650 | 5 |

  **两条干净结论**：①**春季是真金**——一加春季 OA +4.4 分，含春季的两套 10 个种子里 100% 夺冠，没春季的从没赢过；②**地形几乎白加**——S→S+T 只 +1.9 分，而 S+Sp vs S+T+Sp(0.642 vs 0.645)差 0.003，远小于 sd(0.02)=**噪声内打平**。既然打平就选简单的(19<22)，而且**踢掉地形正好消掉 provenance 里两条红旗**(地形 t 检验 p 值因空间自相关无效；地形进分类器 vs 原 spectral-only 计划不一致)。**地形不进分类器，但在 refugia 支线里照样用**(高程/坡度/朝向 vs 烧不烧)。
  - **"全部(S+T+Sp)"当测试过的备选写进报告**："加地形 OA 没提升"——跟 §4 那个诚实套路一致(测了更复杂的，如实报告没更好)。
  - ⚠️ 这**改了报告旧版头条**：旧的 OA=0.684 是 4类**单次 70/30 划分 + S+T(没春季)**；新头条是 4类 **S+Sp + 5折交叉验证**，数字见下面"结果"。数字反而略低但方法更严(5折 CV > 单次划分)，且模型多了 2016 春季这个新日期——春季 provenance 必须正当交代(为什么用火前春季物候、云况/日期怎么选)。
  - ⚠️ **春季的生态解释——写对(逻辑给你，英文你写)**：春季物候主要撬动**松树**(常绿针叶，春天仍高绿→`NDVI_spr` 正向)和**荆豆**(春季黄度/开花→`YI_spr`，**但花期个体差异大，写成"possible reason"不能当定论**)这两类。**阔叶不靠春季**——NZ 本地阔叶灌丛大多**常绿、不落叶不抽叶**，SHAP 显示它靠的是**火前 `NBR_pre` / `pre_B5`(NIR) / `NDMI`**。所以春季让 gorse|broadleaf 那对 JM 大涨，是靠**荆豆随季节变、阔叶不变**这个差异，**不是"阔叶春天抽叶"(这句错的，别写)**。
...
- **可分性**（5）：⭐ 已做（`exploration/spectral_profile_6class.png` + JM矩阵，脚本`scripts/05_separability_jm.py`）。JM指数(0~2，越大越好分)用6波段+NDVI+BSI算：**`bare_rock` 跟其余5类全部完全分开(JM=2.00)**；其余类间大多也分得不错(JM 1.9-2.0)；但 **`gorse_broom` vs `broadleaf_scrub`(JM=1.46)明显是最弱的一对**——光谱曲线图上这两条线在B5/B6/B7几乎重合。这**直接呼应了项目最早期(US1.1 k-means诊断)就担心的那个问题**("native vs gorse分不分得开")，现在用JM给出了定量证据：这两类确实存在真实的光谱混淆风险，§5分类结果如果这两类互相误判多，这里就是原因，不是模型的锅。引用 Richards (2013) *Remote Sensing Digital Image Analysis* 的JM公式。
  - ⭐⭐ **春季把最难那对撬开了（选 S+Sp 最硬的证据，🤖 zip `classification.zip/Tab5_3_JM_distance.csv`）**：`gorse_broom | broadleaf_scrub` 的 JM **只用火前 6 波段=0.85 → 火前+春季 12 波段=1.64**(春季单独 6 波段=0.95)——这对全部 6 对里提升最大。**比 OA 涨几个点更该当主角**：春季直接解决了项目从头到尾最担心的那对光谱混淆。⚠️注意这里的 JM 值(pre 6 bands=0.85)跟上面老点表算的 1.46 不是一个数，因为特征集和点表都换了(新版只用火前 6 波段起步)，以 zip 里这张新表为准。
...
- ⭐⭐ **最终结果（4类 × S+Sp × 5折空间交叉验证，取代下面所有旧数字，🤖 zip `R5.zip`）**：
  - **空间分块 CV(main 171 点，20 次重复)：OA=64.3% ± ~2.0%**(`Tab5_7_cv_summary.csv` / 图 `Fig5_7_confusion_SSp.png`)。各类 PA/UA(`Tab5_15`)：牧草 73%/71%、**荆豆 44%/48%**、阔叶 65%/63%、松树 75%/75%。
  - **独立测试集(expansion 88 点)：OA=59.1%(95% CI 48.6–68.8%)**(`Tab5_14`)。混淆矩阵最大误判仍是**荆豆→阔叶(CV 里 29% 的荆豆跑去阔叶)**——跟 US5 的 JM(0.85)完全对上，是这两类真实的光谱空间重叠，不是模型的锅。
  - ⭐ **掩膜带来的两个发现(🤖 zip `Tab5_13`/`Tab5_17`)**：①有一个"松树"测试点(**OID 10049**)落在 2016 年采伐块里，两次都被分成荆豆——**是标签过期了(2012 LCDB 标松树，2016 已砍)，不是模型错**；去掉它测试 OA 从 59.1%→**59.8%**(变化很小，说明不是它一个点撑起来的)。其余分错的松树点不在 2016 采伐块内(可能更早砍或幼林，可用 GEE 查 Hansen 其他年份损失确认)。②采伐块 300 格里有 **150 格原本被分成荆豆**(采伐剩余物+灌丛看着像荆豆)——**加 cleared_pine 掩膜后荆豆面积从 ~406ha 降到 382ha**(406 来自"全部变量"模型，仅作参考)。**这正好解释了一部分荆豆的错分来源，是"掩膜为什么必要"的直接证据。**
  - **全图面积(掩膜后，`Tab5_17`)**：牧草 543.8ha(30.9%)、荆豆 382.1ha(21.7%)、阔叶 354.1ha(20.1%)、松树 452.5ha(25.7%)、cleared_pine 掩膜 27.0ha(1.5%)。⚠️面积口径：GEE 算的采伐多边形总面积 82ha，但一部分落在格网外，**火场内实际只有 27ha(300 格)，报告用 27ha**。投影用纯 Python 实现，跟 CSV 里的 NZTM 坐标对比误差为 0。
- （↓以下为**旧版迭代记录**，OA 数字已被上面 S+Sp 最终版取代，保留是因为"该学哪几类/掩膜哪几类"的方法学仍然有效）**结果 + 混淆矩阵解读 + 挂文献**（5）：⭐ 6类版本先跑出 OA=0.710/Kappa=0.638(`exploration/rf_confusion_matrix.png`，`scripts/06_random_forest_classify.py`)，但 `bare_rock` PA=0(3个验证点全错，训练点仅7个撑不起任何分类器)。⚠️ 中间版本(5类，剔除bare_rock)一度报出OA=0.742，但那版`cleared_pine`训练点(28个)后来查出26个位置根本不在`exotic_pine`范围内，是位置错误撑起的假象数字，已作废。修正cleared_pine位置(19个点，真实落在exotic_pine内)后单独测5类模型，`cleared_pine`PA仅0.067(15个验证点14个错)——19个点分散在3-4个小图斑，空间分块训练/验证一切，学不出稳定边界，跟bare_rock是同一类问题。**最终版本**：`bare_rock`和`cleared_pine`都从RF训练/验证集里剔除，分类器只学4类真正靠光谱/地形能区分、且样本量足够的植被燃料类型——`scripts/06c_rf_classify_4class_final.py`，图`exploration/rf_confusion_matrix_4class_final.png`。**4类最终结果：OA=0.684，Kappa=0.579**(按Landis&Koch(1977)分级为"moderate agreement"，比6类的0.710略低，但这是把两个"用位置错误数据撑出来的虚高分数"拿掉之后的诚实数字)。混淆矩阵最大的误判仍是 **`gorse_broom`→`exotic_pine`/`broadleaf_scrub`各错3/2个、`broadleaf_scrub`→`gorse_broom`错7个**(20个真实broadleaf_scrub里)——**跟US5的JM指数(gorse_broom vs broadleaf_scrub=1.46，全部15对最低)完全对上**，证实这是这两类本身光谱空间重叠的真实限制。特征重要性：`pre_B4`/`pre_B3`/`pre_B5`最重要，地形变量(`elev`/`northness`)排最后——光谱信息比地形对分类贡献更大。
