...
- **Offset 校正（A1已做，处理季节漂移）**：👤（同一脚本第138-171行）。未烧植被的dNBR理论上
  ...
  拍摄间隔本身带来的读数漂移"，让dNBR=0真正对应"没有变化"，不是碰巧接近0。**③限制**：
  offset本身是控制环的平均值，个别像元(比如刚好在阴影/坡向差异大的控制点)仍可能有残余
  偏差；具体offset数值+校正前后对比的例子(未烧点从异常值修正到接近0)，Yu从A1原始GEE
  console输出里核对后补一句具体数字进来。
...
- **refugia 支线**（US7）：⭐ 已做——4类分类器套到全部22波段栈的每个像元(不切分，用全部176点重新训练，注意这跟上面OA=0.684是两次不同的模型拟合，不能说这张全图逐像素验证过)，裁剪到真实火场边界(`Port_Hills_2017_Fire_Boundary.shp`，NZTM下正确面积1760.9ha，跟训练AOI吻合)。refugia定义=severity==0(dNBR<~100，Key&Benson未燃阈值)。
  - ⭐⭐ **更严谨的阈值定义（A1已做，比"套用Key&Benson通用阈值"更有据）**：👤`00_A1_PortHills_dNBR.js`
    ...
    **③限制**：T的具体数值 + commission error(控制环里被误判成"烧了"的比例)需要Yu从A1
    console输出里核对确切数字后补进来，这里先把方法本身讲清楚。
  - ⭐⭐ **岛屿独立验证（真数字，已核实）**：👤 `PortHills2017_island_validation.csv`(A1的GEE
    ...
    **recall=0.851**(阈值T下85.1%的岛内像元被正确分类为未燃)，岛内平均dNBR=**-13.9**
    ...
    结合地图核实)。
  - ⭐ **未制图refugia发现（A1的真实发现，直答课题核心问题）**：👤 同脚本第233-240行。
    **①做了什么**：用"我的方法找到的未燃总面积"减去"落在官方两块手绘岛内的未燃面积"，
    算出方法找到了多少官方地图**没画出来**的未燃区(unmapped refugia)。**②为什么这重要**：
    官方手绘图只标了两块最明显的岛，但refugia可能是零散分布的小块(比如地形庇护的沟谷)，
    人工目视判读容易漏掉——这条分析直接检验"遥感方法能不能找到人工判读会漏掉的refugia"，
    是refugia科学问题本身的一个真发现，不是复述文献。**③限制**：具体面积数字(ha)+占比
    需要Yu从console输出核对后补充；这类"额外找到的未燃区"也需要肉眼核对几个样点，排除
    是分类误判(比如误把裸岩分成"未燃植被")而不是真refugia的可能性。
