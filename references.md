...
## F. 候选方法（2026-09-21 读 E2/E3 全文挖的 · ⚠️待另一台电脑决定采不采 · 未定前别写进报告）

> 这一节是**菜单不是决定**。Yu 决定采哪条后，才把它落进 workflow.md/report.md 并挂 📖 出处。
> 都是 📖 外部文献可引；下面标了"新活"还是"补引用"。

| # | 候选办法 | 出处 | 对哪节 | 是新活还是补引用 | 采不采 |
|---|---|---|---|---|---|
| F1 | **Spearman 秩相关**并排跑一版（现在是 Pearson+VIF）：不要求正态、抓单调关系，把 §3"多变量非正态"发现和 §4 相关分析串成因果链 | E3 Domingo2020 | §4（连 §3） | ⭐**新活**（唯一真能加分的） | 🔲 |
| F2 | "每个变量单独出图、对照参考影像逐一解读"当 §3 探索的方法背书 | E2 Solares2023 | §3 | 补引用（你已在做 boxplot/hist） | 🔲 |
| F3 | 变量重要性 = mean decrease in Gini 的可引出处；另可加 **drop-one** 法(删一个看 OA 掉多少) | E2 Solares / E3 Domingo | §5 | 补引用 + drop-one 是可选新活 | 🔲 |
...
> ⚠️ **放置待定**：E2/E3 都是同行评审文章，且方法直接对口(ALS+S2 燃料分类)——**可考虑当 Helen 要的"3 篇"或方法先例引**。
> 是否从 E 组升进 A 组，Yu 定。
